...
| Security y permissions | APLICA | cubierta | Sesión HMAC, webhook HMAC, guard anti-troll, validación+cota de `state` (Zod), sin secretos hardcodeados (password demo = sandbox), 0 vulns npm. **CAT-02 (capa LLM v2a):** input tratado como dato (`sanitizeLeadInput`); salida del modelo validada como candidato (`faqId` contra el store) — nunca dirige el estado; `GEMINI_API_KEY` server-side only | XSS, abuso, prompt injection, fuga | `lib/voice/guard.ts`, `lib/voice/llm.ts`, `app/api/voice/turn/route.ts`, reporte `vuln` | ASESOR | GATE 9 |
