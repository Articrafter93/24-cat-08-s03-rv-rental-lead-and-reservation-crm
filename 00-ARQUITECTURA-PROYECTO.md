...
| Security y permissions | APLICA | cubierta | Sesión HMAC, webhook HMAC, guard anti-troll, validación+cota de `state` (Zod), sin secretos hardcodeados (password demo = sandbox), 0 vulns npm. **CAT-02 (capa LLM v2a/v2b):** input tratado como dato (`sanitizeLeadInput`); salida validada — v2a: `faqId` contra el store; v2b: integridad numérica de la reformulación (todo número debe sobrevivir exacto o se rechaza al verbatim); booking/escalación permanecen strings deterministas (no LLM); `GEMINI_API_KEY` server-side only | XSS, abuso, prompt injection, alucinación de hechos, fuga | `lib/voice/guard.ts`, `lib/voice/llm.ts`, `app/api/voice/turn/route.ts`, reporte `vuln` | ASESOR | GATE 9 |
...
| Cost management | APLICA | cubierta | Motor determinista = costo por turno cero; capa LLM v2a opcional (Gemini **free tier**) solo en el camino difícil (FAQ sin match + destino en booking); guard anti-troll acota abuso antes de la API; **sin `GEMINI_API_KEY` = cero llamadas**. Rate-limit/budget de servidor de producción diferido | costo/abuso de IA en producción de tráfico alto | `lib/voice/llm.ts`, `lib/voice/telemetry.ts` (traza `llmUsed`) | CFO | GATE 9 |
